  ...
  cp /dev/null /tmp/logcat.txt
	...
	for i in $(seq 1 $TOTAL_CYCLES)
		do
			...
			adb shell am start-activity -n ${FIRST_ACTIVITY} >> /dev/null
			sleep 1
			adb logcat -d -e Display >> /tmp/logcat.txt
      adb logcat -c
		done

	cp /dev/null /tmp/logcat2.txt
  cat /tmp/logcat.txt | grep ${FIRST_ACTIVITY} >> /tmp/logcat2.txt

  TIMES=$(cat /tmp/logcat2.txt | grep -o '+[0-9]*s*[0-9]*ms' | cut -c 2- | sed 's/[^0-9]*//g' )
  TOTAL_TIME=$(echo $TIMES | awk '{s+=$1}END{print s}' )
  TOTAL_CYCLES=$(echo $TIMES | wc -l )
